...
namespace structures
{
	...
	inline int DSRoutines::indexInVector(std::vector<T>& vector, const T & item)
	{
		...
		//for (std::vector<T>::iterator it = vector.begin(); it != vector.end(); it++) //povodne,nefunguje
		for (typename std::vector<T>::iterator it = vector.begin(); it != vector.end(); it++) //funguje
		//for (auto it = vector.begin(); it != vector.end(); it++) //funguje
		{
			if (*it == item)
			{
				return result;
