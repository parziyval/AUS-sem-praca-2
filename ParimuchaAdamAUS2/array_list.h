...
namespace structures
{
	...
	inline ArrayList<T>::ArrayList():
		List<T>::List(),
		array_(new Array<T>(4)), //vyvtvorime pole o 4 prvkoch
		size_(0)
	{
	}
	...
	inline ArrayList<T>& ArrayList<T>::operator=(const ArrayList<T>& other)
	{
		//test identity
		//zrus stare pole
		//vytvorim nove pole kopirovacim konstruktorom
		//prirad size
		if (this != &other)
		{
			delete array_;
	...
	inline bool ArrayList<T>::tryRemove(const T & data)
	{
		...
		/* alebo takto, ale je to pomalsie, lebo je tam vynimka
		try {
	...
	{
		size_ = 0; //nemusime nic mazat, len posunieme size_, ostatne prvky sa zneplatnia
	}
	...
	{
		Array<T>* new_array_ = new Array<T>(2 * array_->size());
		Array<T>::copy(*array_,0,*new_array_,0,array_->size());
		delete array_;
		array_ = new_array_;
	}
	...
	inline Iterator<T>& ArrayList<T>::ArrayListIterator::operator=(const Iterator<T>& other)
	{
		...
		//netreba kontrolu identity, je to niekde osetrene vraj
	...
	{
		position_++;
		return *this; //vratime objekt , predpis jazyka
	}
