...
namespace structures
{
	...
	public:
		...
		/// <summary> to iste ako findTableItem len je public aby sa daja zavolat </summary>
		TableItem<K, T>* findItem(const K& key) const;
	protected:
	...
	template<typename K, typename T>
	inline TableItem<K, T>* SortedSequenceTable<K, T>::findItem(const K& key) const
	{
		bool found = false;
		int index = this->indexOfKey(key, 0, this->size(), found);
